...
function applyCustomBgInline(hue: number) {
  ...
  el.textContent = `
    ...
    body::before {
      background:
        radial-gradient(ellipse 60% 50% at 15% 25%, ${light}  0%, transparent 65%),
        radial-gradient(ellipse 50% 60% at 85% 55%, ${dark}   0%, transparent 60%),
        radial-gradient(ellipse 70% 40% at 50% 85%, ${light3} 0%, transparent 65%),
        radial-gradient(ellipse 40% 30% at 70% 15%, ${light2} 0%, transparent 55%),
        linear-gradient(160deg, ${bg} 0%, ${mid} 40%, ${bg} 100%) !important;
    }
...
export default function SettingsPage() {
  ...
  function handleBgHueDrag(hue: number) {
    setBgHue(hue); setCustomBgOn(true);
  ...
  async function handleSendFeedback() {
    ...
    if (error) {
      ...
      setFeedbackRating(0); setFeedbackMessage(""); setFeedbackFeature("");
    }
  ...
  async function handleDeleteAccount() {
    ...
    try {
      ...
      if (json.dryRun) {
        ...
        setDeleteStatus("Dry run complete — check browser console. (SAFETY_LOCK is still on)");
    ...
    <div className="shell">
      <div className="container-app space-y-5">
              ...
              return (
                <button key={size.id} type="button"
                  ...
                  <span className="font-bold leading-none text-white"
                    style={{ fontSize: `${10 + FONT_SIZES.indexOf(size) * 3}px` }}>Aa</span>
                  <span className="text-[9px] font-medium"
                    style={{ color: isActive ? "#FDE68A" : "rgba(255,255,255,0.4)" }}>{size.label}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="overflow-hidden rounded-3xl"
          ...
          <button type="button"
            onClick={() => { setShowDeleteConfirm((v) => !v); setDeleteInput(""); setDeleteStatus(""); }}
