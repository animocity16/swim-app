...
function getStrokeLabel(event: string): string {
  const e = event.toLowerCase();
  if (e.includes("freestyle")) return "FREE";
  if (e.includes("backstroke")) return "BACK";
  if (e.includes("breaststroke")) return "BREAST";
  if (e.includes("butterfly")) return "FLY";
...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    if (recentData && recentData.length > 0) {
      ...
      const { data: betterTimes } = await supabase
        ...
        .lt("time_ms", row.time_ms)
        .limit(1);

      const isPB = !betterTimes || betterTimes.length === 0;
      setLatestResult({ ...row, swimmer_name: swimmer?.name ?? "Unknown", is_pb: isPB });
    ...
    if (setsData && setsData.length > 0 && allTimes) {
      const setId = setsData[0].id;
      const { data: items } = await supabase
        .from("standard_items")
        .select("id, event, course, qualifying_time_ms")
  ...
  const primarySwimmer = swimmers[0];
          ...
          {swimmers.length > 1 && (
            <div className="mt-4 pt-4 border-t border-white/10 flex gap-2 flex-wrap">
              {swimmers.slice(1).map((s, i) => {
                ...
                return (
                  <Link key={s.id} href={`/swimmers/${s.id}`}
                ...
                {standardsSummary.qualified === standardsSummary.total ? (
                  ...
                  <>
                    <p className="text-lg font-bold text-white">
                      {standardsSummary.qualified} qualified
                    </p>
                  ...
                  <>
                    ...
                    <p className="text-xs text-white/45 mt-0.5">
                      {standardsSummary.inProgress} events in progress
                    </p>
                  </>
                )}
              </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"
                        ...
                        {getStrokeLabel(latestResult.event)}
                      </span>
                      {latestResult.is_pb && (
                        <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"
                    ...
                    <p className="text-xs text-white/35 mt-0.5">
                      {latestResult.course}
                      {latestResult.meet_name ? ` · ${latestResult.meet_name}` : ""}
                  ...
                  {deltaMs !== null && Math.abs(deltaMs) > 0 && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-white/35">
                        over {sparklineTimes.length} swims
                      </span>
                    </div>
                  )}
                </div>
              ...
              {recentPBs.map((pb) => {
                ...
                return (
                  <Link key={`${pb.swimmer_id}-${pb.event}-${pb.course}`}
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-wider mb-1" style={{ color: strokeColor }}>
                      {getStrokeLabel(pb.event)}
                    </p>
