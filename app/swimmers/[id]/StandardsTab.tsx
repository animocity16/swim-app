  ...
  function relevantItems(setId: number): StandardItem[] {
    return items.filter((item) => {
      ...
      if (
        item.gender &&
        swimmerGender &&
        item.gender.toLowerCase() !== swimmerGender.toLowerCase()
      )
        return false;
    ...
    return (
      ...
      >
        ...
        <p className="text-xs text-white/40 mb-4">
          Add upgrading times or meet qualifying standards to track progress.
        </p>
        <Link
          href="/standards"
      ...
      {visibleSets.map((set) => {
        ...
        return (
          <div key={set.id}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {total === 0 ? (
                    ...
                    <p className="text-xs text-white/40 mt-0.5">
                      {qualified} qualified · {attempted - qualified} in progress ·{" "}
                      {total - attempted} not yet swum
                    </p>
                  )}
                </div>
                <svg
            ...
            {isExpanded && displayItems.length === 0 && (
              <div
                className="mt-1 rounded-3xl p-4 text-center"
                style={{ border: "1px solid rgba(255,255,255,0.07)", background: "rgba(0,10,30,0.4)" }}
              >
                ...
                <Link
                  href={`/standards/${set.id}`}
                  className="text-[10px] font-semibold mt-2 block"
                  style={{ color: "rgba(253,230,138,0.5)" }}
                >
                  Edit standard set →
                </Link>
